...
// wo_roroutine_mgr is based on virtual machine's runtime
// wo_roroutine_thread_mgr will based on Fiber/UContext
//

#include "wo_roroutine_thread.hpp"
#include "wo_vm.hpp"
...
namespace wo
{
    ...
    class fvmscheduler_fwaitable_base : public wo::fwaitable
    {
    ...
    public:
        ...
        ~vmthread()
        {
            ...
            wo_asure(m_virtualmachine->interrupt(vmbase::PENDING_INTERRUPT));
        }
        vmthread(wo::vmbase* _vm, wo_int_t vm_funcaddr, size_t argc, wo::shared_pointer<RSCO_Waitter> _waitter = nullptr)
        {
        ...
        vmthread(wo::vmbase* _vm, closure_t* vm_funcaddr, size_t argc, wo::shared_pointer<RSCO_Waitter> _waitter = nullptr)
        {
            m_virtualmachine = _vm;
            if (_vm->co_pre_invoke(vm_funcaddr, argc))
            {
        ...
        {
            auto _old_this_thread_vm = wo::vmbase::_this_thread_vm;
            wo::vmbase::_this_thread_vm = m_virtualmachine;

            m_fthread->invoke_from(from_fib);

            wo::vmbase::_this_thread_vm = _old_this_thread_vm;
        }
    ...
    class fvmscheduler
    {
        ...
        class fvmshcedule_queue_thread
        {
            ...
            static void _queue_thread_work(fvmshcedule_queue_thread* _this)
            {
                ...
                SetThreadDescription(GetCurrentThread(), L"wo_coroutine_worker_thread");
#endif
                fiber _queue_main;
        ...
        public:
            ...
            wo::shared_pointer<RSCO_Waitter> new_work(vmbase* _vm, FT funcaddr, size_t argc)
            {
                std::lock_guard g1(_jobs_queue_mx);
                if (_shutdown_flag)
                    return nullptr;
            ...
            {
                wo_assert(_paused_flag && !_current_vmthread);
        ...
        {
#ifdef WO_PLATRORM_OS_WINDOWS
            SetThreadDescription(GetCurrentThread(), L"wo_coroutine_scheduler_thread");
#endif
            using namespace std;
            for (; !_this->_shutdown_flag;)
            {
        ...
        {
#ifdef WO_PLATRORM_OS_WINDOWS
            SetThreadDescription(GetCurrentThread(), L"wo_coroutine_timer_thread");
#endif
            using namespace std;
            for (; !_this->_shutdown_flag;)
            {
            ...
            , _hr_current_time_point(_hr_clock.now())
        {
            ...
            wo_assert(working_thread_count);
        }

        inline static fvmscheduler* _scheduler = nullptr;

    public:
        static wo::shared_pointer<fvmscheduler_waitfortime> wait(double _tm)
        {
            using namespace std;
        ...
        static void init(size_t working_thread_count = 4)
        {
            ...
            {
                wo_assert(_scheduler == nullptr, "_scheduler should be nullptr here.");
                _scheduler = new fvmscheduler(working_thread_count);
            }
            else
                wo_warning("_scheduler disabled!");
        }
        ...
        template<typename FT>
        static wo::shared_pointer<RSCO_Waitter> new_work(vmbase* _vm, FT funcaddr, size_t argc)
        {
            if (!_scheduler)
            {
                wo_fail(WO_FAIL_NOT_SUPPORT, "Coroutine scheduler not supported.");
                return nullptr;
