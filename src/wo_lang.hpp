...
namespace wo
{
    ...
    public:
        ...
        struct pass1_action
        {
            ...
            union
            {
                grammar::ast_base* node;
                lang_scope* scope;

            };
            pass1_action(lang_scope* _scope)
                : scope(_scope)
                , action_type(
                    _scope->type == lang_scope::scope_type::namespace_scope
                    ? LEAVE_NAMESPACE
                    : _scope->type == lang_scope::scope_type::function_scope
                    ? LEAVE_FUNCTION
                    : LEAVE_SCOPE)
            {}
            pass1_action(grammar::ast_base* _node)
                : node(_node)
                , action_type(APPEND_AST_NODE)
            {}
        };

        void collect_ast_nodes_for_pass1(grammar::ast_base* begin_ast_node)
        {
            using namespace ast;

            std::vector<grammar::ast_base*> _pass1_analyze_list;
            std::stack<pass1_action> _pending_action;
            std::stack<pass1_action> _temp_pending_action;

#define PASS1_ANALYZE(NODE) do{ pass1_action a = NODE; a.action_type = pass1_action::ANALYZE_AST_NODE; _temp_pending_action.push(a); }while(0)
#define PASS1_BEGIN_NAMESPACE begin_namespace
#define PASS1_BEGIN_FUNCTION begin_function

#define PASS1_END_SCOPE [&](lang_scope* s) { _temp_pending_action.push(pass1_action(s)); };

            // Append first node.
            PASS1_ANALYZE(begin_ast_node);
            _pending_action.emplace(_temp_pending_action.top());
            _temp_pending_action.pop();
            wo_assert(_temp_pending_action.empty());

            while (!_pending_action.empty())
            {
                auto&& cur_action = std::move(_pending_action.top());
                _pending_action.pop();

                switch (cur_action.action_type)
                {
                case pass1_action::LEAVE_NAMESPACE: end_namespace(); continue;
                case pass1_action::LEAVE_FUNCTION: end_function(); continue;
                case pass1_action::LEAVE_SCOPE: end_scope(); continue;
                case pass1_action::APPEND_AST_NODE:
                    if (_pass1_analyze_list.end() == std::find(_pass1_analyze_list.begin(), _pass1_analyze_list.end(), cur_action.node))
                        _pass1_analyze_list.push_back(cur_action.node); 
                    continue;
                default:break;
                }

                wo_assert(cur_action.action_type == pass1_action::ANALYZE_AST_NODE);
                auto* cur_node = cur_action.node;

                if (_pass1_analyze_list.end() != std::find(_pass1_analyze_list.begin(), _pass1_analyze_list.end(), cur_node))
                    // Cur node has been append to list. return!
                    continue;

                //////////////////////// Begin Collect

                if (ast_symbolable_base* a_symbol_ob = dynamic_cast<ast_symbolable_base*>(cur_node))
                    a_symbol_ob->searching_begin_namespace_in_pass2 = now_scope();

                //// Begin collect different type of node
                if (ast_namespace* a_namespace = dynamic_cast<ast_namespace*>(cur_node))
                {
                    auto * namesapce = PASS1_BEGIN_NAMESPACE(a_namespace->scope_name);
                    PASS1_ANALYZE(a_namespace->in_scope_sentence);

                    PASS1_END_SCOPE(namesapce);
                }
                else if (ast_varref_defines* a_varref_defs = dynamic_cast<ast_varref_defines*>(cur_node))
                {
                    for (auto& varref : a_varref_defs->var_refs)
                    {
                        PASS1_ANALYZE(varref.init_val);
                        PASS1_ANALYZE(a_varref_defs->declear_attribute);
                        PASS1_ANALYZE(varref.pattern);
                    }
                }
                else if (ast_value_binary* a_value_bin = dynamic_cast<ast_value_binary*>(cur_node))
                {
                    PASS1_ANALYZE(a_value_bin->left);
                    PASS1_ANALYZE(a_value_bin->right);
                }
                else if (ast_value_index* a_value_idx = dynamic_cast<ast_value_index*>(cur_node))
                {
                    PASS1_ANALYZE(a_value_idx->from);
                    PASS1_ANALYZE(a_value_idx->index);
                }
                else if (ast_value_assign* a_value_assi = dynamic_cast<ast_value_assign*>(cur_node))
                {
                    PASS1_ANALYZE(a_value_assi->left);
                    PASS1_ANALYZE(a_value_assi->right);
                }
                else if (ast_value_logical_binary* a_value_logic_bin = dynamic_cast<ast_value_logical_binary*>(cur_node))
                {
                    PASS1_ANALYZE(a_value_logic_bin->left);
                    PASS1_ANALYZE(a_value_logic_bin->right);
                }
                else if (ast_value_logical_binary* a_value_logic_bin = dynamic_cast<ast_value_logical_binary*>(cur_node))
                {
                    PASS1_ANALYZE(a_value_logic_bin->left);
                ...
                else if (ast_value_variable* a_value_var = dynamic_cast<ast_value_variable*>(cur_node))
                {
                    // TODO;
                }
                else if (ast_value_type_cast* a_value_cast = dynamic_cast<ast_value_type_cast*>(cur_node))
                {
                    PASS1_ANALYZE(a_value_cast->_be_cast_value_node);
                }
                else if (ast_value_type_judge* ast_value_judge = dynamic_cast<ast_value_type_judge*>(cur_node))
                {
                    PASS1_ANALYZE(ast_value_judge->_be_cast_value_node);
                }
                else if (ast_value_type_check* ast_value_check = dynamic_cast<ast_value_type_check*>(cur_node))
                {
                    PASS1_ANALYZE(ast_value_check->_be_check_value_node);
                }
                else if (ast_value_function_define* a_value_func = dynamic_cast<ast_value_function_define*>(cur_node))
                {
                    a_value_func->this_func_scope = PASS1_BEGIN_FUNCTION(a_value_func);
                    ...
                    PASS1_END_SCOPE(a_value_func->this_func_scope);
                ...
                //////////////////////// Add self to list;
                _temp_pending_action.push(pass1_action(cur_node)); // Used for append cur node into list
            }
        }
