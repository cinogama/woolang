...
namespace wo
{
    ...
    lang_ValueInstance::Storage::Storage(woort_IRStaticIndex static_index, bool is_pvalue_indirect)
        : m_type(GLOBAL)
        ...
        lexer& lex, LangContext* ctx, ast::AstTypeHolder* type_holder)
    {
        ...
        case ast::AstTypeHolder::IDENTIFIER:
        {
            ...
            if (symbol == m_dictionary || symbol == m_mapping)
            {
                ...
                {
                    lex.record_lang_error(lexer::msglevel_t::error, type_holder,
                        diagnose::lang1::err_unexpected_template_count{ (size_t)2, template_arguments.size() });
            ...
            else if (symbol == m_array || symbol == m_vector)
            {
                ...
                {
                    lex.record_lang_error(lexer::msglevel_t::error, type_holder,
                        diagnose::lang1::err_unexpected_template_count{ (size_t)1, template_arguments.size() });
            ...
            {
                lex.record_lang_error(lexer::msglevel_t::error, type_holder,
                    diagnose::lang2::err_cannot_use_builtin_typename_here{symbol});
    ...
    compile_result LangContext::process(lexer& lex, ast::AstBase* root)
    {
        ...
        // NOTE: Donot need to generate entry recheck code for REPL program.
        if (!m_repl_context.has_value())
        {
        ...
        // Pre-passir sweep (REPL only): allocate IR storage and emit
        // initializer code for template value instances whose pattern-
        // declaration AST is absent from the current eval's tree. In REPL
        ...
        // before any usage code generated by the passir traversal below.
        // This is safe because cross-eval template initializers only
        // reference symbols from their declaring scope (prior eval / import),
        // never locals introduced later in the current eval.
        //
        // The passir pattern handler (AstVariableDefineItem) is adapted to
        // skip instances that already have IR_storage, so same-eval instances
        ...
        for (;;)
        {
            ...
            for (ast::AstValueFunction* eval_function : functions_to_finalize)
            {
                ...
                eval_function->m_IR_function_MUST_BE_CLEAR_FOR_REPL.emplace(
                ...
                if (m_repl_context.has_value())
                    m_repl_context.value()->emit_script_func(eval_function);
        ...
        while (!scanning_scopes.empty())
        {
            ...
            for (auto& [name, symbol] : this_scope->m_defined_symbols)
            {
                ...
                lex.record_lang_error(lexer::msglevel_t::error,
                    ...
                    diagnose::lang2::err_unused_variable{symbol.get()});
            ...
            lexer& lex, ast::AstIdentifier* ident, const std::optional<bool*>& out_ambig)
    {
        ...
        if (found_symbol.size() > 1)
        {
            ...
            lex.record_lang_error(lexer::msglevel_t::error, ident,
                diagnose::lang2::err_ambiguous_target_named{ident->m_name->c_str(), result});
            ...
            for (auto* symbol : found_symbol)
            {
                if (symbol->m_symbol_declare_ast.has_value())
                    lex.record_lang_error(lexer::msglevel_t::infom,
                        symbol->m_symbol_declare_ast.value(),
                        diagnose::lang2::info_maybe_named_defined_here{symbol});
                else
                    lex.record_lang_error(lexer::msglevel_t::infom,
                        ident,
                        diagnose::lang2::info_maybe_named_defined_in_compiler{symbol});
            }
            ...
            lexer& lex, ast::AstIdentifier* ident, const std::optional<bool*>& out_ambig)
    {
        ...
        case ast::AstIdentifier::identifier_formal::FROM_TYPE:
        {
                ...
                from_type != nullptr)
            {
                ...
                if (!determined_type)
                {
                    lex.record_lang_error(lexer::msglevel_t::error, *from_type, diagnose::parser::err_unknown_type{});
                    return std::nullopt;
                }
        ...
        std::optional<ast::AstTemplateConstantTypeCheckInPass1*> template_checker)
    {
        ...
        for (; params_iter != params_end; ++params_iter, ++args_iter)
        {
                ...
                if (argument.m_constant.has_value())
                {
                        ...
                        && argument.m_type->m_symbol == m_origin_types.m_nothing.m_symbol)
                    {
                        ...
                        lex.record_lang_error(lexer::msglevel_t::error, param,
                            diagnose::lang1::err_this_template_arg_should_not_be_nothing{});
            ...
            {
                lex.record_lang_error(lexer::msglevel_t::error, param,
                    diagnose::parser::err_redefined{param->m_param_name->c_str()});
