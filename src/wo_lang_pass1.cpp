...
namespace wo
{
    ...
    static const char* binary_operator_symbol(AstValueBinaryOperator::operator_type op)
    {
        switch (op)
        {
        case AstValueBinaryOperator::ADD: return "+";
        case AstValueBinaryOperator::SUBSTRACT: return "-";
        case AstValueBinaryOperator::MULTIPLY: return "*";
        case AstValueBinaryOperator::DIVIDE: return "/";
        case AstValueBinaryOperator::MODULO: return "%";
        case AstValueBinaryOperator::LOGICAL_AND: return "&&";
        case AstValueBinaryOperator::LOGICAL_OR: return "||";
        case AstValueBinaryOperator::GREATER: return ">";
        case AstValueBinaryOperator::GREATER_EQUAL: return ">=";
        case AstValueBinaryOperator::LESS: return "<";
        case AstValueBinaryOperator::LESS_EQUAL: return "<=";
        case AstValueBinaryOperator::EQUAL: return "==";
        case AstValueBinaryOperator::NOT_EQUAL: return "!=";
        }
        return "?";
    }

    static const char* unary_operator_symbol(AstValueUnaryOperator::operator_type op)
    {
                    ...
                    == tuple_pattern->m_fields.size())
                {
                    ...
                    for (uint16_t idx = 0; pattern_iter != pattern_end; ++pattern_iter, ++type_iter, ++idx)
                    {
                        ...
                        if (constant_tuple_value.has_value())
                            constant_elem_value = &constant_tuple_value.value()->value_struct().m_elements[idx];
        ...
        if (symbol_instance->m_symbol_declare_location.has_value())
        {
                ...
                symbol_location_may_null->source_group, path))
            {
                lex.record_lang_error(lexer::msglevel_t::error, node,
                    ...
                    get_symbol_name(symbol_instance),
        ...
        wo_pstring_t path)
    {
        ...
        if (struct_type_inst->m_symbol_declare_ast.has_value())
        {
            lex.record_lang_error(lexer::msglevel_t::infom, struct_type_inst->m_symbol_declare_ast.value(),
                WO_INFO_SYMBOL_NAMED_DEFINED_HERE,
                get_symbol_name(struct_type_inst));
        }
        return false;
    }
    ...
    void LangContext::init_pass1()
    {
        ...
        WO_LANG_REGISTER_PROCESSER(AstEchoForREPL, AstBase::AST_ECHO_FOR_REPL, pass1);
                        ...
                        if (type_symbol->m_symbol_kind == lang_Symbol::kind::ALIAS)
                        {
                                ...
                                && identifier->m_template_arguments.has_value())
                            {
                                ...
                                if (!refiliing_symbol->m_is_template && !refiliing_symbol->m_is_builtin)
                                {
                                    lex.record_lang_error(lexer::msglevel_t::error, node,
                                        WO_ERR_UNEXPECTED_TEMPLATE_ARGUMENT,
                                        get_symbol_name(refiliing_symbol));

                                    lex.record_lang_error(lexer::msglevel_t::infom, node,
                                        WO_INFO_TRYING_REFILL_TEMPLATE_ARGUMENT,
                                        get_type_name(alias_instance->m_determined_type.value()),
                                        get_symbol_name(alias_instance->m_symbol));
    ...
    WO_PASS_PROCESSER(AstWhereConstraints)
    {
        ...
        else if (state == HOLD)
        {
            ...
            for (auto& constraint : node->m_constraints)
            {
                ...
                if (!constraint->m_evaled_const_value.value().value_bool())
                {
                    failed = true;
                    lex.record_lang_error(lexer::msglevel_t::error, constraint,
                ...
                for (auto& [param_type, argument_type] : node->m_LANG_constant_check_pairs)
                {
                        ...
                        lex, param_type, param_type->m_LANG_determined_type.value(), argument_type))
                    {
                        ...
                        lex.record_lang_error(lexer::msglevel_t::error, param_type,
                            WO_ERR_TYPE_NOT_ACCEPTED_NAMED,
                            get_type_name(argument_type),
                            get_type_name(param_type->m_LANG_determined_type.value()));
                    }
                }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            lex.record_lang_error(lexer::msglevel_t::error, node,
                                ...
                                get_type_name(last_function_return_type),
                                get_type_name(return_value_type));
                ...
                for (; element_iter != element_end; ++element_iter)
                {
                        ...
                        != is_type_accepted(lex, element, array_elemnet_type, element_type))
                    {
                        lex.record_lang_error(lexer::msglevel_t::error,
                            ...
                            get_type_name(array_elemnet_type));
                        return FAILED;
                    }
                }
            }
                ...
                for (++element_iter; element_iter != element_end; ++element_iter)
                {
                        ...
                        != is_type_accepted(lex, element, key_type, element_key_type))
                    {
                        lex.record_lang_error(lexer::msglevel_t::error, element,
                            ...
                            get_type_name(element_key_type),
                            get_type_name(key_type));
                        return FAILED;
                    }
                        ...
                        != is_type_accepted(lex, element, value_type, element_value_type))
                    {
                        lex.record_lang_error(lexer::msglevel_t::error, element,
                            ...
                            get_type_name(element_value_type),
    ...
    WO_PASS_PROCESSER(AstValueTuple)
    {
        ...
        else if (state == HOLD)
        {
            ...
            for (auto& element : node->m_elements)
            {
                ...
                if (element->node_type == AstBase::AST_FAKE_VALUE_UNPACK)
                {
                    ...
                    if (unpack->m_unpack_value->m_evaled_const_value.has_value())
                    {
                        ...
                        auto& struct_constant = unpacking_constant_tuple.value_struct();
                        for (uint16_t idx = 0; idx < struct_constant.m_count; ++idx)
                        {
            ...
            if (element_constants.size() == element_types.size())
            {
                ...
                node->decide_final_constant_value(element_constants);
            }
        }
        return WO_EXCEPT_ERROR(state, OKAY);
    }
        ...
        else if (state == HOLD)
        {
            ...
            case AstValueIndex::HOLD_FOR_RIGHT_OPNUM_EVAL:
            {
                ...
                case lang_TypeInstance::DeterminedType::STRUCT:
                {
                    ...
                    {
                        lex.record_lang_error(lexer::msglevel_t::error, node->m_index,
                            WO_ERR_STRUCT_DO_NOT_HAVE_MEMBER_NAMED,
                            get_type_name(container_type_instance),
                            member_name->c_str());
                        return FAILED;
                    }
        ...
        else if (state == HOLD)
        {
            ...
            if (!unpack_value_type_determined_base_type)
            {
                lex.record_lang_error(lexer::msglevel_t::error, node->m_unpack_value,
                    WO_ERR_TYPE_NAMED_DETERMINED_FAILED,
                    get_type_name(unpack_value_type_instance));
                return FAILED;
            }
        ...
        else if (state == HOLD)
        {
            ...
            if (!variable_instance->m_mutable)
            {
                lex.record_lang_error(lexer::msglevel_t::error, node,
                    ...
                    get_value_name(variable_instance));
                return FAILED;
            }
        }
        return WO_EXCEPT_ERROR(state, OKAY);
    ...
    WO_PASS_PROCESSER(AstValueTypeCast)
    {
        ...
        else if (state == HOLD)
        {
            ...
            case AstValueMayConsiderOperatorOverload::HOLD_FOR_OVERLOAD_FUNCTION_CALL_EVAL:
            {
                ...
                {
                    lex.record_lang_error(lexer::msglevel_t::error, node,
                        WO_ERR_OPERATOR_AS_RESULT_TYPE_NOT_ACCEPTED_NAMED,
                ...
                else
                {
                        ...
                        != lang_TypeInstance::DeterminedType::FUNCTION)
                    {
                        ...
                        lex.record_lang_error(lexer::msglevel_t::error, node->m_function,
                            ...
                            get_type_name(target_function_type_instance));
                        return FAILED;
                    }
            ...
            case AstValueFunctionCall::HOLD_BRANCH_B_TEMPLATE_ARGUMENT_DEDUCTION:
            {
                ...
                if (argument_types.size() < target_function_param_types.size())
                {
                    lex.record_lang_error(lexer::msglevel_t::error, node,
                        ...
                        argument_types.size(),
                        target_function_param_types.size());
            ...
            case AstValueFunctionCall::HOLD_FOR_FUNCTION_EVAL:
                if (node->m_is_direct_call)
                {
                    ...
                    lex.record_lang_error(lexer::msglevel_t::infom, first_argument,
                        ...
                        get_type_name(first_argument->m_LANG_determined_type.value()));
                }
                break;
            case AstValueFunctionCall::HOLD_BRANCH_A_TEMPLATE_ARGUMENT_DEDUCTION:
            {
                ...
                && immutable_type(target_type) != m_origin_types.m_void.m_type_instance)
            {
                    ...
                    != check_cast_able(lex, node, target_type, value_type))
                {
                    lex.record_lang_error(lexer::msglevel_t::error, node->m_check_type,
                        WO_ERR_CANNOT_CAST_TYPE_NAMED_FROM_DYNAMIC,
                        get_type_name(target_type));
        ...
        else if (state == HOLD)
        {
            ...
            case AstValueStruct::HOLD_TO_STRUCT_TYPE_CHECK:
            {
                if (node->m_marked_struct_type.has_value())
                {
                    ...
                    if (!determined_base_type.has_value())
                    {
                        lex.record_lang_error(lexer::msglevel_t::error, node,
                            WO_ERR_TYPE_NAMED_DETERMINED_FAILED,
                            get_type_name(struct_type_instance));
            ...
            case AstValueStruct::HOLD_FOR_FIELD_EVAL:
            {
                ...
                if (!struct_determined_base_type.has_value())
                {
                    lex.record_lang_error(lexer::msglevel_t::error, node,
                        WO_ERR_TYPE_NAMED_DETERMINED_FAILED,
                        get_type_name(struct_type_instanc));
                ...
                if (node->m_fields.size() < struct_type_info->m_member_types.size())
                {
                    ...
                    for (const auto& member_entry : struct_type_info->m_member_types)
                    {
                        auto member_name = member_entry.first;
                        bool provided = false;
                        for (auto& field : node->m_fields)
                        {
                            if (field->m_name == member_name)
                            {
                                provided = true;
                                break;
                            }
                        ...
                        if (!provided)
                        {
                            if (!missing_fields.empty())
                                missing_fields += ", ";
                            missing_fields += member_name->c_str();
                        }
                    }
                ...
                for (auto* field : node->m_fields)
                {
                            ...
                            field->m_value->m_LANG_determined_type.value()))
                    {
                        lex.record_lang_error(lexer::msglevel_t::error, field->m_value,
                            WO_ERR_TYPE_NOT_ACCEPTED_NAMED,
                            get_type_name(field->m_value->m_LANG_determined_type.value()),
                            get_type_name(accpet_field_type));
            ...
            case AstValueBinaryOperator::HOLD_FOR_RIGHT_OPNUM_EVAL:
            {
                ...
                if (left_type != right_type)
                {
                    lex.record_lang_error(lexer::msglevel_t::error, node,
                        ...
                        binary_operator_symbol(node->m_operator));
                ...
                if (node->m_left->m_evaled_const_value.has_value())
                {
                    auto& left_value = node->m_left->m_evaled_const_value.value();
                    switch (node->m_operator)
                    {
                    case AstValueBinaryOperator::LOGICAL_AND:
                        if (left_value.value_bool() == 0)
                        {
                            node->decide_final_constant_value(false);
                    ...
                    case AstValueBinaryOperator::LOGICAL_OR:
                        if (left_value.value_bool() != 0)
                        {
                            node->decide_final_constant_value(true);
                            constant_has_been_determined = true;
                        }
                        break;
                    default:
                    ...
                    && node->m_right->m_evaled_const_value.has_value())
                {
                    ...
                    auto& left_value = node->m_left->m_evaled_const_value.value();
                    auto& right_value = node->m_right->m_evaled_const_value.value();
                    wo_assert(left_value.m_type == right_value.m_type);
                    ...
                    case AstValueBinaryOperator::LOGICAL_AND:
                        node->decide_final_constant_value(
                            (bool)left_value.value_bool() && (bool)right_value.value_bool());
                        break;
                    case AstValueBinaryOperator::LOGICAL_OR:
                        node->decide_final_constant_value(
                            (bool)left_value.value_bool() || (bool)right_value.value_bool());
            ...
            case AstValueUnaryOperator::NEGATIVE:
            {
                    ...
                    && detrmined_base_type->m_base_type != lang_TypeInstance::DeterminedType::REAL)
                {
                    lex.record_lang_error(lexer::msglevel_t::error, node->m_operand,
                        WO_ERR_UNACCEPTABLE_TYPE_IN_OPERATE,
                        get_type_name(operand_type),
                        unary_operator_symbol(node->m_operator));
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            lex.record_lang_error(lexer::msglevel_t::error, node,
                                WO_ERR_UNABLE_TO_MIX_TYPES,
                                get_type_name(true_type_instance),
                                get_type_name(false_type_instance));
            ...
            case AstMatch::HOLD_FOR_EVAL_MATCHING_VALUE:
            {
                ...
                if (!determined_base_type.has_value())
                {
                    ...
                    lex.record_lang_error(lexer::msglevel_t::error, node->m_matched_value,
                        WO_ERR_TYPE_NAMED_DETERMINED_FAILED,
                        get_type_name(matching_typeinstance));
                    ...
                    != lang_TypeInstance::DeterminedType::UNION)
                {
                    ...
                    lex.record_lang_error(lexer::msglevel_t::error, node->m_matched_value,
                        WO_ERR_UNEXPECTED_MATCHING_TYPE,
                        get_type_name(matching_typeinstance));
                    ...
                    && covered_branch.size() != determined_base_type_instance_union_dat->m_union_label.size())
                {
                    ...
                    for (const auto& label_entry : determined_base_type_instance_union_dat->m_union_label)
                    {
                        auto label_name = label_entry.first;
                        if (covered_branch.find(label_name) == covered_branch.end())
                        {
                            if (!missing_cases.empty())
                                missing_cases += ", ";
                            missing_cases += label_name->c_str();
                        }
                    }
