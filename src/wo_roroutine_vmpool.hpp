...
public:
    ...
    {
        wo::gcbase::rw_lock m_vm_list_mx;
        std::vector<wo::vmbase*> m_vm_list;
    ...
    public:
        ~RSCO_PoolVMList()
        {
            ...
            {
                while (
                    !(vmm->vm_interrupt & wo::vmbase::vm_interrupt_type::LEAVE_INTERRUPT)
                    && (!(vmm->vm_interrupt & wo::vmbase::vm_interrupt_type::PENDING_INTERRUPT)
                        || !(vmm->vm_interrupt & wo::vmbase::vm_interrupt_type::ABORT_INTERRUPT)))
                    std::this_thread::yield();
                delete vmm;
            }
        }
        wo::vmbase* get_usable_vm(wo::vmbase* origin)
        {
            do
            {
                std::shared_lock s1(m_vm_list_mx);
                for (auto* vmm : m_vm_list)
                    if (vmm->clear_interrupt(wo::vmbase::vm_interrupt_type::PENDING_INTERRUPT))
                    {
                        vmm->sp = vmm->bp = vmm->stack_mem_begin;
                        vmm->clear_interrupt(wo::vmbase::ABORT_INTERRUPT);
                        return vmm;
                    }
            } while (0);

            std::lock_guard g1(m_vm_list_mx);
            wo::vmbase* vmm = origin->make_machine(1024);
            return m_vm_list.emplace_back(vmm);
        }
        wo::shared_pointer<wo::runtime_env> get_one_env()
        {
            std::shared_lock s1(m_vm_list_mx);
        ...
        bool need_recude()
        {
                ...
                env && env.used_count() == m_vm_list.size() + 2/*(GC_THREAD)*/)
            {
                ...
                {
                    if (!(vm->vm_interrupt & wo::vmbase::vm_interrupt_type::PENDING_INTERRUPT))
                        return false;
                }
                return true;
            }
...
private:
    wo::gcbase::rw_lock m_pooled_vm_list_mx;
    std::map<wo::runtime_env*, RSCO_PoolVMList> m_pooled_vm_list;
...
public:
    ...
    static wo::vmbase* get_usable_vm(wo::vmbase* origin)
    {
        wo_assert(origin && origin->env);
    ...
    static void do_reduce()
    {
        ...
        std::list<wo::runtime_env*> need_reduce_vm;
