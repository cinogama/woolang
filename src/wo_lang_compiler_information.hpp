...
// PASS LEXER
#define WO_ERR_UNEXPECTED_CH_AFTER_CH u8"数字字面量中出现了非法字符 `%c`"
#define WO_ERR_UNEXPECTED_EOF u8"意外的文件结束"
#define WO_ERR_UNEXPECTED_TOKEN u8"意外的符号: `%s`"
#define WO_ERR_ILLEGAL_LITERAL u8"数字字面量后出现非法字符 `%c`"
#define WO_ERR_UNKNOWN_OPERATOR_STR u8"未知的运算符: `%s`"
#define WO_ERR_UNEXPECTED_EOL_IN_CHAR u8"字符常量中不允许换行符"
#define WO_ERR_NO_CHAR_IN_CHAR u8"字符常量至少要有一个字符"
#define WO_ERR_TOO_MANY_CHAR_IN_CHAR u8"字符常量只能包含一个字符"
#define WO_ERR_UNEXPECTED_EOL_IN_STRING u8"字符串常量中不允许换行符"
...
#define WO_ERR_UNKNOWN_ESCSEQ_BEGIN_WITH_CH u8"未知的转义序列: `\\%c`"
...
#define WO_ERR_REPEAT_ATTRIBUTE u8"特性修饰符重复"
#define WO_ERR_FAILED_TO_CREATE_TUPLE_WITH_VAARG u8"元组类型中不能使用可变参数 `...`"
#define WO_ERR_INVALID_KEY_EXPR u8"映射的键必须是单个值表达式"
...
#define WO_ERR_EXTERN_LIB_SHOULD_BE_CONSTANT u8"指定的外部库名应当是一个常量"
...
#define WO_ERR_STRUCT_DO_NOT_HAVE_MEMBER_NAMED u8"结构体 `%s` 没有成员 `%s`"
#define WO_ERR_TUPLE_INDEX_OUT_OF_RANGE u8"元组 `%s` 只有 %zu 个元素, 索引 %lld 越界"
#define WO_ERR_PATTERN_VARIABLE_SHOULD_BE_MUTABLE u8"不能对不可变变量 `%s` 赋值"
...
#define WO_ERR_ARGUMENT_TOO_MUCH u8"函数调用参数过多: 提供了 %zu 个参数, 但只接受 %zu 个"
#define WO_ERR_ARGUMENT_TOO_LESS u8"函数调用参数不足: 提供了 %zu 个参数, 但需要 %zu 个"
#define WO_ERR_TYPE_NOT_ACCEPTED_NAMED u8"类型不匹配: 无法将类型 `%s` 用于此处 (期望 `%s`)"
#define WO_ERR_OPERATOR_AS_RESULT_TYPE_NOT_ACCEPTED_NAMED u8"类型不匹配: operator as 的返回类型 `%s` 无法转换到目标类型 `%s`"
#define WO_ERR_CANNOT_CAST_TYPE_NAMED_FROM_DYNAMIC u8"无法在运行时将 dynamic 值转换为类型 `%s`"
#define WO_ERR_TYPE_NAMED_IS_NOT_STRUCT u8"类型错误: `%s` 不是结构体类型"
#define WO_ERR_REPEATED_FIELD_NAMED u8"结构体字段重复: 字段名 `%s` 已被使用"
#define WO_ERR_NOT_ALL_FIELD_INITIALIZED u8"结构体初始化不完整，缺少以下字段: %s"
...
#define WO_ERR_DIFFERENT_TYPE_IN_BINARY u8"二元运算符 `%s` 两侧的操作数类型不匹配"
#define WO_ERR_UNACCEPTABLE_TYPE_IN_OPERATE u8"类型 `%s` 不支持 `%s` 运算"
...
#define WO_ERR_UNEXPECTED_MATCHING_TYPE u8"匹配类型错误: match 语句只支持 union 类型 (当前为 `%s`)"
#define WO_ERR_TAKEPLACE_PATTERN_MATCHED u8"匹配顺序错误: 占位模式(`_`)必须放在最后"
#define WO_ERR_ALL_CASES_SHOULD_BE_MATCHED u8"匹配不完整: 必须覆盖所有可能的分支，缺少以下分支: %s"
#define WO_ERR_UNEXISTS_CASE_NAMED_IN_MATCH u8"匹配项不存在: `%s` 不是 union 类型 `%s` 的成员"
...
#define WO_ERR_CANNOT_USE_BUILTIN_TYPENAME_HERE u8"内置类型名 `%s` 不能在此处作为类型使用"
...
#define WO_ERR_DEFER_CANNOT_BE_HERE u8"`defer` 语句必须位于可执行的作用域（函数体或代码块）内"
...
#define WO_ERR_VARIABLE_STORAGE_NOT_DETERMINED u8"引用了未完成初始化的变量 `%s`"
...
#define WO_ERR_UNUSED_VARIABLE u8"局部变量 `%s` 已声明但未使用"
