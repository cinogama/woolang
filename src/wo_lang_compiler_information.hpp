...
#define WO_TOO_MANY_ERROR(count) \
    reinterpret_cast<const char*>(u8"错误数量过多(已达上限), 仅显示前 ")\
    + std::to_string(count)\
    + reinterpret_cast<const char*>(u8" 条错误, 编译终止")
...
#define WO_ERR_UNKNOWN_MACRO_NAMED reinterpret_cast<const char*>(u8"未定义的宏 `%s`")
...
#define WO_ERR_BAD_BREAK reinterpret_cast<const char*>(u8"break 语句只能在循环或 switch 中使用")
...
#define WO_INFO_BAD_LABEL_NAMED reinterpret_cast<const char*>(u8"找不到标签为 `%s` 的循环")
