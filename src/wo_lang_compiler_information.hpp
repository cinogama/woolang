...
#define WO_ERR_LINE_NEED_STRING_AS_PATH L"此处需要一个字符串字面量作为路径"
#define WO_ERR_LINE_NEED_INTEGER_AS_ROW L"此处需要一个整数字面量作为行位置"
#define WO_ERR_LINE_NEED_INTEGER_AS_COL L"此处需要一个整数字面量作为列位置"
...
#define WO_ERR_FAILED_TO_DEDUCE_NOT_FUNC_PARAM_TYPE L"一个泛型函数在这里试图匹配函数类型，但此处目标参数类型并非函数，而是 '%ls'"
...
#define WO_ERR_THIS_TEMPLATE_ARG_SHOULD_NOT_BE_NOTHING L"不能使用 nothing 常量作为值的泛型参数"
