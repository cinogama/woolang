    ...
    size_t* out_count)
{
    ...
    // Phase 1: Walk expression collection for identifier and type references.
    for (auto& [file_name, expr_map] : meta->m_source_expr_collection)
    {
        for (auto& [begin_loc, inner_map] : expr_map)
        {
            for (auto& [end_loc, ast_base] : inner_map)
            {
                wo_lspv2_semantic_token token = {};
                token.m_token_type = WO_LSPV2_SEMANTIC_VARIABLE;
                token.m_modifiers = 0;

                if (ast_base->node_type == wo::ast::AstBase::node_type_t::AST_TYPE_HOLDER)
                {
                    token.m_token_type = WO_LSPV2_SEMANTIC_TYPE;
                }
                else if (ast_base->node_type == wo::ast::AstBase::node_type_t::AST_VALUE_VARIABLE)
                {
                    wo::ast::AstValueVariable* var =
                        static_cast<wo::ast::AstValueVariable*>(ast_base);
                    if (var->m_LANG_variable_instance.has_value())
                    {
                        auto* vi = var->m_LANG_variable_instance.value();
                        if (vi->m_symbol->m_symbol_kind == wo::lang_Symbol::kind::TYPE)
                            token.m_token_type = WO_LSPV2_SEMANTIC_TYPE;
                        else if (vi->m_symbol->m_symbol_kind == wo::lang_Symbol::kind::ALIAS)
                            token.m_token_type = WO_LSPV2_SEMANTIC_TYPE;
                        else
                            token.m_token_type = WO_LSPV2_SEMANTIC_VARIABLE;
                    }
                ...
                token.m_begin_row = begin_loc.row;
                token.m_begin_col = begin_loc.column;
                token.m_end_row = end_loc.row;
                token.m_end_col = end_loc.column;
        ...
        [&](wo::lang_Scope* scope)
    {
        for (auto& [name, symbol] : scope->m_defined_symbols)
        {
            ...
            wo_lspv2_semantic_token token = {};
            token.m_begin_row = loc.begin_at.row;
            token.m_begin_col = loc.begin_at.column;
            token.m_end_row = loc.end_at.row;
            token.m_end_col = loc.end_at.column;
            token.m_modifiers = WO_LSPV2_SEMANTIC_MOD_DECLARATION;
        ...
        for (auto& sub_scope : scope->m_sub_scopes)
            walk_scope(sub_scope.get());

        if (scope->is_namespace_scope())
        {
            for (auto& [ns_name, ns] : scope->m_belongs_to_namespace->m_sub_namespaces)
                walk_scope(ns->m_this_scope.get());
        }
    };
    ...
    {
        wo_lspv2_semantic_token token = {};
        token.m_begin_row = macro->m_location.begin_at.row;
        token.m_begin_col = macro->m_location.begin_at.column;
        token.m_end_row = macro->m_location.end_at.row;
        token.m_end_col = macro->m_location.end_at.column;
        token.m_token_type = WO_LSPV2_SEMANTIC_MACRO;
        token.m_modifiers = WO_LSPV2_SEMANTIC_MOD_DECLARATION;
...
void wo_lspv2_semantic_tokens_free(wo_lspv2_semantic_token* tokens)
{
    delete[] tokens;
}
