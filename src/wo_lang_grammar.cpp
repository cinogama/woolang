...
namespace wo
{
    grammar* get_wo_grammar()
    {
        ...
        {
#endif
            wo_stdout << ANSI_HIY "RSGramma: " ANSI_RST "Syntax update detected, LR table cache is being regenerating..." << wo_endl;
            ...
            wo_stdout << ANSI_HIY "RSGramma: " ANSI_RST "Checking LR(1) table..." << wo_endl;

            if (wo_grammar->check_lr1())
            {
                wo_stdout << ANSI_HIR "RSGramma: " ANSI_RST "LR(1) have some problem, abort." << wo_endl;
                exit(-1);
            }
            ...
            if (!WO_GRAMMAR_SKIP_GEN_LR1_TABLE_CACHE)
            {
                ...
                wo_stdout << ANSI_HIY "RSGramma: " ANSI_RST "OK, now writting cache..." << wo_endl;
                ...
                {
                    wo_stdout << ANSI_HIR "RSGramma: " ANSI_RST "File will be generated to current work path, may have some problem..." << wo_endl;
                    exit(-1);
                }

                std::wofstream cachefile(cur_path);

                wo_stdout << ANSI_HIY "RSGramma: " ANSI_RST "Write to " << cur_path << wo_endl;
                ...
                for (auto& [aim, rule] : wo_grammar->ORGIN_P)
                {
                    ...
                    {
                        wo_wstdout << ANSI_HIY "RSGramma: " ANSI_RST "Producer: " ANSI_HIR << grammar::lr_item{ grammar::rule{ aim ,rule },size_t(-1),grammar::te(grammar::ttype::l_eof) }
                ...
                wo_stdout << ANSI_HIG "RSGramma: " ANSI_RST "Finished." << wo_endl;
            }
            else
            {
                wo_stdout << ANSI_HIG "RSGramma: " ANSI_RST "Skip generating LR(1) table cache (WO_GRAMMAR_SKIP_GEN_LR1_TABLE_CACHE is true)." << wo_endl;
            }
