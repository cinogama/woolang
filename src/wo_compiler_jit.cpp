    ...
    struct asmjit_compiler_x64 : public asmjit_backend<X64CompileContext>
    {
            ...
            runtime_env* env)
        {
            if (dr)
            {
                ...
                if ((*rt_ip) & (1 << 7))
                {
                    ...
                    wo_asure(!x86compiler.lea(result, asmjit::x86::qword_ptr(stack_bp, WO_SIGNED_SHIFT(WO_IPVAL_MOVE_1) * sizeof(value))));
                ...
                else
                {
                    // from reg
                    auto result = x86compiler.newUIntPtr();
                    wo_asure(!x86compiler.lea(result, asmjit::x86::qword_ptr(reg, WO_IPVAL_MOVE_1 * sizeof(value))));
                    return may_constant_x86Gp{ &x86compiler,false,nullptr,result };
                }
            }
            else
            ...
            asmjit::x86::Gp rt_tc)
        {
            ...
            else
            {
                ...
                wo_asure(!x86compiler.mov(bpoffset, asmjit::x86::qword_ptr(vm, offsetof(vmbase, stack_mem_begin))));
                wo_asure(!x86compiler.sub(bpoffset, rt_bp));
                wo_asure(!x86compiler.mov(asmjit::x86::dword_ptr(rt_sp, offsetof(value, vmcallstack) + offsetof(value::callstack, bp)), bpoffset.r32()));

                auto callargptr = x86compiler.newUIntPtr();
                auto targc = x86compiler.newInt64();
                wo_asure(!x86compiler.lea(callargptr, asmjit::x86::qword_ptr(rt_sp, 1 * (int32_t)sizeof(value))));
                wo_asure(!x86compiler.lea(targc, asmjit::x86::qword_ptr(rt_tc, offsetof(value, integer))));
            ...
            runtime_env* env)override
        {
            ...
            *out_compiler = &ctx->c;
            ...
            wo_asure(!ctx->c.mov(ctx->_vmreg, asmjit::x86::qword_ptr(ctx->_vmbase, offsetof(vmbase, register_mem_begin))));
            wo_asure(!ctx->c.mov(ctx->_vmcr, asmjit::x86::qword_ptr(ctx->_vmbase, offsetof(vmbase, cr))));
            wo_asure(!ctx->c.mov(ctx->_vmtc, asmjit::x86::qword_ptr(ctx->_vmbase, offsetof(vmbase, tc))));
...
#undef WO_JIT_NOT_SUPPORT
    ...
    struct AArch64CompileContext
    {
        ...
        asmjit::a64::Mem load_int64_mem(wo_integer_t val)
        {
            return c.newInt64Const(asmjit::ConstPoolScope::kLocal, val);
        }
        asmjit::a64::Gp load_int64(wo_integer_t val)
        {
            ...
            auto gp = c.newInt64();
            c.ldr(gp, load_int64_mem(val));
            return m_int_constant_pool[val] = gp;
        }
        asmjit::a64::Mem load_float64_mem(wo_real_t val)
        {
            return c.newDoubleConst(asmjit::ConstPoolScope::kLocal, val);
        }
        asmjit::a64::Vec load_float64(wo_real_t val)
        {
            ...
            auto dvec = c.newVecD();
            c.ldr(dvec, load_float64_mem(val));
    ...
    struct asmjit_compiler_aarch64 : public asmjit_backend<AArch64CompileContext>
    {
        struct aarch64_addresing
        {
                ...
                : m_constant(nullptr)
            {
                ...
                _m_addr = context->c.newIntPtr();
                if (m_type == addressing_type::BPOFFSET)
                {
                    wo_asure(!context->c.mov(_m_addr, m_offset));
                    wo_asure(!context->c.add(_m_addr, _m_addr, context->_vmsbp));
                }
                else if (m_type == addressing_type::REGISTER)
                {
                    wo_asure(!context->c.mov(_m_addr, m_offset));
                    wo_asure(!context->c.add(_m_addr, _m_addr, context->_vmreg));
                }
                else
                {
                    wo_asure(!context->c.mov(_m_addr, m_offset));
                    wo_asure(!context->c.add(_m_addr, _m_addr, context->_vmglb));
                }
            }
            asmjit::a64::Gp get_addr()const
            {
                return _m_addr;
            }
        };
            ...
            asmjit::a64::Gp rt_tc)
        {
            ...
            else
            {
                ...
                wo_asure(!ctx->c.strb(type, asmjit::a64::Mem(rt_sp, offsetof(value, type))));
                auto bpoffset = ctx->c.newUInt64();
                wo_asure(!ctx->c.ldr(bpoffset, asmjit::a64::Mem(vm, offsetof(vmbase, stack_mem_begin))));
                wo_asure(!ctx->c.sub(bpoffset, bpoffset, rt_bp));
                wo_asure(!ctx->c.str(bpoffset.w(), asmjit::a64::Mem(rt_sp, offsetof(value, vmcallstack) + offsetof(value::callstack, bp))));

                wo_asure(!ctx->c.sub(bpoffset, rt_ip, codes));
                wo_asure(!ctx->c.str(bpoffset.w(), asmjit::a64::Mem(rt_sp, offsetof(value, vmcallstack) + offsetof(value::callstack, ret_ip))));

                auto callargptr = ctx->c.newUIntPtr();
                auto targc = ctx->c.newInt64();
                wo_asure(!ctx->c.add(callargptr, rt_sp, sizeof(value)));
                wo_asure(!ctx->c.ldr(targc, asmjit::a64::Mem(rt_tc, offsetof(value, integer))));

                asmjit::InvokeNode* invoke_node;

                wo_asure(!ctx->c.invoke(&invoke_node, jitcode_holder,
                    asmjit::FuncSignatureT<wo_result_t, vmbase*, value*, size_t>()));
            ...
            runtime_env* env)override
        {
            ...
            *out_compiler = &ctx->c;
            ...
            wo_asure(!ctx->c.mov(ctx->_vmssp, ctx->_vmsbp));                    // let sp = bp;
            wo_asure(!ctx->c.ldr(ctx->_vmglb, asmjit::a64::Mem(ctx->_vmbase, offsetof(vmbase, const_global_begin))));
            wo_asure(!ctx->c.ldr(ctx->_vmreg, asmjit::a64::Mem(ctx->_vmbase, offsetof(vmbase, register_mem_begin))));
            wo_asure(!ctx->c.ldr(ctx->_vmcr, asmjit::a64::Mem(ctx->_vmbase, offsetof(vmbase, cr))));
            wo_asure(!ctx->c.ldr(ctx->_vmtc, asmjit::a64::Mem(ctx->_vmbase, offsetof(vmbase, tc))));
        ...
        static void a64_set_val(AArch64CompileContext* ctx, asmjit::a64::Gp target, asmjit::a64::Gp val)
        {
            ...
            wo_asure(!ctx->c.str(tmp, asmjit::a64::Mem(target, offsetof(value, integer))));
            tmp = ctx->c.newGpw();
            wo_asure(!ctx->c.ldrb(tmp, asmjit::a64::Mem(val, offsetof(value, type))));
            wo_asure(!ctx->c.strb(tmp, asmjit::a64::Mem(target, offsetof(value, type))));
        }
...
#define WO_JIT_NOT_SUPPORT do{ return false; }while(0)
            ...
            ctx->generate([=]()
                {
                    if (opnum2.m_constant != nullptr)
                        a64_set_imm(ctx, opnum1.get_addr(), opnum2.m_constant);
                    else
                        a64_set_val(ctx, opnum1.get_addr(), opnum2.get_addr());
                });
            ...
            ctx->generate([=]()
                {
                    ...
                    wo_asure(!ctx->c.str(tmp, asmjit::a64::Mem(opnum1.get_addr(), offsetof(value, integer))));
                });
            return true;
        }
            ...
            ctx->generate([=]()
                {
                    ...
                    wo_asure(!ctx->c.sub(tmp, tmp, tmp2));
                    wo_asure(!ctx->c.str(tmp, asmjit::a64::Mem(opnum1.get_addr(), offsetof(value, integer))));
                });
            return true;
        }
            ...
            ctx->generate([=]()
                {
                    ...
                    wo_asure(!ctx->c.mul(tmp, tmp, tmp2));
                    wo_asure(!ctx->c.str(tmp, asmjit::a64::Mem(opnum1.get_addr(), offsetof(value, integer))));
                });
            return true;
        }
        ...
        virtual bool ir_addr(AArch64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            auto tmp = ctx->c.newVecD();
            auto tmp2 = opnum2.m_constant != nullptr
                ? ctx->load_float64(opnum2.m_constant->integer)
                : ctx->c.newVecD();
            ctx->generate([=]()
                {
                    wo_asure(!ctx->c.ldr(tmp, asmjit::a64::Mem(opnum1.get_addr(), offsetof(value, real))));
                    if (opnum2.m_constant == nullptr)
                        wo_asure(!ctx->c.ldr(tmp2, asmjit::a64::Mem(opnum2.get_addr(), offsetof(value, real))));

                    wo_asure(!ctx->c.fadd(tmp, tmp, tmp2));
                    wo_asure(!ctx->c.str(tmp, asmjit::a64::Mem(opnum1.get_addr(), offsetof(value, real))));
                });
            return true;
        }
            ...
            ctx->generate([=]()
                {
                    ...
                    wo_asure(!ctx->c.fsub(tmp, tmp, tmp2));
                    wo_asure(!ctx->c.str(tmp, asmjit::a64::Mem(opnum1.get_addr(), offsetof(value, real))));
                });
            return true;
        }
            ...
            ctx->generate([=]()
                {
                    ...
                    wo_asure(!ctx->c.fmul(tmp, tmp, tmp2));
                    wo_asure(!ctx->c.str(tmp, asmjit::a64::Mem(opnum1.get_addr(), offsetof(value, real))));
                });
            return true;
        }
            ...
            ctx->generate([=]()
                {
                    ...
                    wo_asure(!ctx->c.fdiv(tmp, tmp, tmp2));
                    wo_asure(!ctx->c.str(tmp, asmjit::a64::Mem(opnum1.get_addr(), offsetof(value, real))));
                });
            return true;
        }
        ...
        virtual bool ir_adds(AArch64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            auto jitcall_adds = ctx->load_int64((intptr_t)&_vmjitcall_adds);

            ctx->generate([=]()
                {
                    asmjit::InvokeNode* invoke_node;
                    wo_asure(!ctx->c.invoke(&invoke_node, (intptr_t)&jitcall_adds,
                        asmjit::FuncSignatureT<void, wo::value*, wo::value*>()));

                    invoke_node->setArg(0, opnum1.get_addr());
                    invoke_node->setArg(1, opnum2.get_addr());
                });
            return true;
        }
                ...
                ctx->generate([=]()
                    {
                        if (opnum1.m_constant != nullptr)
                            a64_set_imm(ctx, ctx->_vmssp, opnum1.m_constant);
                        else
                            a64_set_val(ctx, ctx->_vmssp, opnum1.get_addr());
                ...
                ctx->generate([=]()
                    {
                        for (uint32_t i = 0; i < psh_repeat; i++)
                        {
                            a64_set_imm(ctx, ctx->_vmssp, &NIL_VALUE);
                            wo_asure(!ctx->c.sub(ctx->_vmssp, ctx->_vmssp, sizeof(value)));
                        }});
            }
            return true;
        ...
        virtual bool ir_pop(AArch64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            if (dr & 0b01)
            {
                ...
                ctx->generate([=]()
                    {
                        wo_asure(!ctx->c.add(ctx->_vmssp, ctx->_vmssp, sizeof(value)));
                        a64_set_val(ctx, opnum1.get_addr(), ctx->_vmssp);
                    });
            }
            else
            {
                size_t pop_count = (size_t)WO_IPVAL_MOVE_2;
                ctx->generate([=]()
                    {
                        wo_asure(!ctx->c.add(ctx->_vmssp, ctx->_vmssp, pop_count * sizeof(value)));
                    });
            }
            return true;
        }
        ...
        virtual bool ir_gti(AArch64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            ctx->generate([=]() {
                ...
                wo_asure(!ctx->c.cmp(tmp, tmp2));
                wo_asure(!ctx->c.b_le(x86_cmp_fail));
        ...
        virtual bool ir_calln(AArch64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            else
            {
                ...
                auto codeholder_addr = ctx->load_int64((int64_t)compiled_funcstat->m_func);
                auto codeholder = ctx->c.newIntPtr();
                ctx->c.ldr(codeholder, asmjit::a64::Mem(codeholder_addr));
                ctx->generate([=] {
        ...
        virtual bool ir_jt(AArch64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            ctx->generate([=]() {
                ...
                wo_asure(!ctx->c.b_ne(jump_ip(&ctx->c, jmp_place)));
                });
            return true;
        }
