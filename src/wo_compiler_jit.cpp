...
#undef WO_JIT_NOT_SUPPORT
        ...
        static wo_result_t native_do_calln_nativefunc(vmbase* vm, wo_extern_native_func_t call_aim_native_func, const byte_t* codes, const byte_t* rt_ip, value* rt_sp, value* rt_bp)
        {
            rt_sp->type = value::valuetype::callstack;
            rt_sp->vmcallstack.ret_ip = (uint32_t)(rt_ip - codes);
        ...
        static wo_result_t native_do_calln_vmfunc(vmbase* vm, wo_extern_native_func_t call_aim_native_func, const byte_t* codes, const byte_t* rt_ip, value* rt_sp, value* rt_bp)
        {
            rt_sp->type = value::valuetype::callstack;
            rt_sp->vmcallstack.ret_ip = (uint32_t)(rt_ip - codes);
            rt_sp->vmcallstack.bp = (uint32_t)(vm->stack_mem_begin - rt_bp);
        ...
        static wo_result_t native_do_call_vmfunc(vmbase* vm, value* target_function, const byte_t* codes, const byte_t* rt_ip, value* rt_sp, value* rt_bp)
        {
            ...
            case value::valuetype::handle_type:
                return native_do_calln_vmfunc(vm, (wo_extern_native_func_t)(void*)target_function->handle, codes, rt_ip, rt_sp, rt_bp);
            case value::valuetype::closure_type:
            {
                ...
                return native_do_calln_vmfunc(vm, target_function->closure->m_native_func, codes, rt_ip, rt_sp, rt_bp);
            }
            default:
    ...
    struct asmjit_compiler_x64 : public asmjit_backend<X64CompileContext>
    {
        ...
        static void make_checkpoint(asmjit::x86::Compiler& x86compiler, asmjit::x86::Gp rtvm, asmjit::x86::Gp stack_sp, asmjit::x86::Gp stack_bp, const byte_t* ip)
        {
            ...
            wo_asure(!x86compiler.cmp(result, asmjit::Imm(wo_result_t::WO_API_NORMAL)));
            wo_asure(!x86compiler.je(no_interrupt_label));
            wo_asure(!x86compiler.ret(result)); // break this execute!!!
            ...
            asmjit::x86::Gp rt_bp)
        {
            ...
            wo_asure(!x86compiler.invoke(&invoke_node, (intptr_t)&native_do_calln_nativefunc,
                asmjit::FuncSignatureT<wo_result_t, vmbase*, wo_extern_native_func_t, const byte_t*, const byte_t*, value*, value*>()));

            invoke_node->setArg(0, vm);
            invoke_node->setArg(1, asmjit::Imm((size_t)call_aim_native_func));
            invoke_node->setArg(2, asmjit::Imm((size_t)codes));
            ...
            wo_asure(!x86compiler.cmp(result, asmjit::Imm(wo_result_t::WO_API_NORMAL)));
            wo_asure(!x86compiler.je(normal));
            wo_asure(!x86compiler.ret(result)); // break this execute!!!
            wo_asure(!x86compiler.bind(normal));
        }
            ...
            asmjit::x86::Gp rt_tc)
        {
            ...
            wo_asure(!x86compiler.shr(bpoffset, asmjit::Imm(4)));
            ...
            if (vm_func->m_finished)
            {
                ...
                wo_asure(!x86compiler.invoke(&invoke_node, *vm_func->m_func,
        ...
        {
            asmjit::InvokeNode* invoke_node;
            wo_asure(!x86compiler.invoke(&invoke_node, (intptr_t)&_vmjitcall_fail,
...
#define WO_JIT_NOT_SUPPORT do{ return false; }while(0)
        ...
        virtual bool ir_psh(X64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            else
            {
                ...
                if (psh_repeat > 0)
                {
                    wo_asure(!ctx->c.sub(ctx->_vmssp, (psh_repeat - 1) * sizeof(value)));

                    asmjit::InvokeNode* invoke_node;
                    wo_asure(!ctx->c.invoke(&invoke_node, (intptr_t)&memset,
        ...
        virtual bool ir_sidarr(X64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            wo_asure(!ctx->c.cmp(err, asmjit::Imm(0)));
            wo_asure(!ctx->c.je(noerror_label));

            x86_do_fail(ctx->c, ctx->_vmbase,
                WO_FAIL_INDEX_FAIL, err,
                rt_ip, ctx->_vmssp, ctx->_vmsbp);

            wo_asure(!ctx->c.bind(noerror_label));
            return true;
        }
        ...
        virtual bool ir_ltr(X64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            wo_asure(!ctx->c.jae(x86_cmp_fail));
        ...
        virtual bool ir_eltr(X64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            wo_asure(!ctx->c.ja(x86_cmp_fail));
        ...
        virtual bool ir_egtr(X64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            wo_asure(!ctx->c.jb(x86_cmp_fail));
        ...
        virtual bool ir_call(X64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            wo_asure(!ctx->c.invoke(&invoke_node, (intptr_t)&native_do_call_vmfunc,
                asmjit::FuncSignatureT<wo_result_t, vmbase*, value*, const byte_t*, const byte_t*, value*, value*>()));
            ...
            wo_asure(!ctx->c.cmp(result, asmjit::Imm(wo_result_t::WO_API_NORMAL)));
            wo_asure(!ctx->c.je(normal));
            wo_asure(!ctx->c.ret(result)); // break this execute!!!
            wo_asure(!ctx->c.bind(normal));
        ...
        virtual bool ir_movcast(X64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            asmjit::InvokeNode* invoke_node;
            wo_asure(!ctx->c.invoke(&invoke_node, (intptr_t)&vm::movcast_impl,
        ...
        {
            asmjit::InvokeNode* invoke_node;
            wo_asure(!ctx->c.invoke(&invoke_node, (intptr_t)&vm::make_closure_fast_impl,
        ...
        virtual bool ir_idstr(X64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            asmjit::InvokeNode* invoke_node;
            wo_asure(!ctx->c.invoke(&invoke_node, (intptr_t)&_vmjitcall_idstr,
                asmjit::FuncSignatureT< void, wo::value*, wo::value*, wo::value*>()));
        ...
        virtual bool ir_equr(X64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            wo_asure(!ctx->c.jp(x86_cmp_fail));
            wo_asure(!ctx->c.jne(x86_cmp_fail));
        ...
        virtual bool ir_nequr(X64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            wo_asure(!ctx->c.jp(x86_cmp_fail));
            wo_asure(!ctx->c.je(x86_cmp_fail));
        ...
        virtual bool ir_equs(X64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            asmjit::InvokeNode* invoke_node;
            wo_asure(!ctx->c.invoke(&invoke_node, (intptr_t)&_vmjitcall_equs,
                asmjit::FuncSignatureT<void, wo::value*, wo::value*, wo::value*>()));
        ...
        virtual bool ir_nequs(X64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip)override
        {
            ...
            asmjit::InvokeNode* invoke_node;
            wo_asure(!ctx->c.invoke(&invoke_node, (intptr_t)&_vmjitcall_nequs,
                asmjit::FuncSignatureT<void, wo::value*, wo::value*, wo::value*>()));
        ...
        virtual bool ir_ext_packargs(X64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip) override
        {
            ...
            asmjit::InvokeNode* invoke_node;
            wo_asure(!ctx->c.invoke(&invoke_node, (intptr_t)&vm::packargs_impl,
        ...
        virtual bool ir_ext_unpackargs(X64CompileContext* ctx, unsigned int dr, const byte_t*& rt_ip) override
        {
            ...
            asmjit::InvokeNode* invoke_node;
            wo_asure(!ctx->c.invoke(&invoke_node, (intptr_t)&vm::unpackargs_impl,
