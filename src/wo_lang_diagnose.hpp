// COMPILER DIAGNOSES - SHARED INFRASTRUCTURE AND EARLY-STAGE PAYLOADS
//
// This is the home of the diagnose infrastructure (lang_diagnose_t /
// diagnose_model_t / format / is_diagnose_t), the message-fragment macros
// (WO_MSG_*) and every diagnostic payload that carries no lang-instance
// fields. Only a forward declaration of LangContext is needed, so this
// header is pulled in at the top of the include chain by
...
// (PASS AST BUILDER) and wo::diagnose::lang1 (lang-stage payloads carrying
// no lang-instance fields). The shared infrastructure (lang_diagnose_t /
...
// symbol) - wo::diagnose::lang2 - live in wo_lang_diagnose_lang.hpp: their
// render() calls into LangContext members, so that header is only included
// by the lang-stage translation units, after wo_lang.hpp's types are
// complete.
#pragma once
...
// wo_lang_diagnose_lang.hpp, namespace wo::diagnose::lang2.)
